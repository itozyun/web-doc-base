 ...
 * 
 * @param {!Element} elm
 * @param {!function(this:Element, !Event):void} callback
 */
 p_listenWheelEvent = function( elm, callback ){
    if( WheelEvent_USE_WHEEL ){
        ...
        for( var pairs = WheelEvent_TARGET_ELEMENT_AND_CALLBACK_PIARS, i = 0, l = pairs.length; i < l; i += 2 ){
...
var WheelEvent_USE_WHEEL               = 9 <= p_Trident || p_EdgeHTML || 17 <= p_Gecko || p_Goanna || 31 <= p_Chromium || 537.7 <= p_WebKit /* Safari 7+ */ || 7 <= p_SafariMobile;
...
if( WheelEvent_USE_DOMMOUSESCROLL ){
    /**
     * [2n+0] !Element = EventTarget = event.currentTarget
     * [2n+1] calback
     * @type {!Array.<!Element|!function(this:Element, !Event):void>}
     */
    ...
    var WheelEvent_onDOMMouseScroll = function( e ){
        var pairs     = WheelEvent_TARGET_ELEMENT_AND_CALLBACK_PIARS,
            elmTarget = /** @type {!Element} */ (e.target),
            i = 0, l = pairs.length, elmCurrentTarget;

        for( ; i < l; i += 2 ){
            elmCurrentTarget = /** @type {!Element} */ (pairs[ i ]);
            if( elmCurrentTarget === elmTarget || p_DOM_contains( elmCurrentTarget, elmTarget ) ){
                /** @type {!function(this:Element, !Event):void} */ (pairs[ i + 1 ]).call( elmCurrentTarget, e );
            };
        };
    };
};
