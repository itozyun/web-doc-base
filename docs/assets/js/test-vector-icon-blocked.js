...
(function(Va, Y, jb, Fc, O, ic, jd, Wb, qb, jc, kc, Gc, lc, r) {
      ...
      if (5.5 > B) {
        ...
        if (b) {
          for (A = -1, m = b.split(";"); F = m[++A];) {
            var R = F.split(":")[0];
            oa(a, R, F.substr(R.length + 1));
          }
        }
      } else if (7.1 > J) {
        if (a.setAttribute("style", ""), b) {
          for (m = b.split(";"); F = m[++A];) {
            F = F.split(":"), oa(a, F[0], F[1]);
          }
        }
      } else {
  ...
  (function() {
    ...
    function ea(d) {
      ...
      (d = ia(u, Ba)) ? (D("[webFontTest] testWebFont mesurement success : " + d), I(d)) : qa(Ba) ? (D("[webFontTest] testWebFont timeout!"), Ia ? pa() : Ga ? I(0) : rb !== r ? ca(rb) : oc(ca)) : ba(ea);
    }
    function ia(d, v) {
      var q = 0, p = -1, k = [];
      if (!h) {
      ...
      ab(ya);
      5 > B && (ya = r);
      v && (q || qa(v)) && D("[webFontTest] " + d + " " + k.join(", "));
      return q;
    }
    function ca(d) {
      D("[webFontTest] onTestDataURIComplete : " + d);
      d ? (Ia = !0, Ba = N, D("[webFontTest] webFontTest_repeatToTestDataURIedCSSWebFont start!"), u = u.replace(oa, ""), pa()) : I(0);
    }
    function pa() {
      if (w) {
        for (; w.length;) {
    ...
    var Ga = 9 > B, xa = 6 > za || J || 530 > ta || 5 > Ea || Fa(11), oa = "bad_" + eb() + "_", V = 5E3, N = 500, aa = "mmmmmmmmmmlliiiiiiiii", ma = 9 <= za && 18 > za || 3 > $a || 7 > Ea || ta && 0 >= Va.conpare(Za, "536.11"), a = ["monospace", "sans-serif", "serif"];
  ...
  (function() {
    ...
    if (Z && 0 > Va.conpare(Za, "0.9.5")) {
      ...
      var L = gb ? function(f) {
        16 === f.keyCode ? qa = !0 : 9 === f.keyCode && ca && oa(ca, !0) && (ca.blur && ca.blur(), f.stopPropagation(), f.preventDefault(), ba(N));
      } : function(f) {
        16 === f.keyCode ? qa = !0 : 9 === f.keyCode && (ea = eb());
      }, Ga = function(f) {
        16 === f.keyCode && (qa = !1);
      }, xa = gb ? function(f) {
        ...
        qa && (u || w - 99 < ea && ea <= w) && (U = "previousSibling", ka = "lastChild");
        for (ia = r; f !== C;) {
          if (Ba) {
            w = f;
            var Ba = !1;
          ...
          if (nb(d, "prettyprint") && !nb(d, "prettyprinted")) {
            ...
            if (!v) {
              ...
              if (!q) {
                ...
                var k;
                !q && (k = pa(d)) && "CODE" === bb(k) && (q = h(k.className, "lang-") || h(k.className, "language-"));
              }
              p = bb(d);
